...
/* Step 7 Hovers For More Information 
	// Start by appending a div to the body that will contain the hover content
	// Take a look at the finished hover - we need 5 lines of similar content, so we'll create a function to fill in those lines
	// Fill in the correct arguments to populate each line with the correct content
	// Create two functions for dv.update.hover - show and hide
	// The show function uses d3.event to correctly place the hover just offset from the mouse
	// Update the content in the hover to contain the actual values for the item we're hovering over
	// The hover will be passed all of the circle's data from within a function(d) { dv.update.hover.show(d); }  
	// Use the formatters to display the numbers in the hover nicely
	// Update the dv.draw.bubbles function to include event listeners for mouseover and mouseout that reference this function


// Adds the hover div and fills in all the placeholder content
dv.draw.hover = function() {
	dv.html.hover = {};
	dv.html.hover.main = d3.select('CSS SELECTOR').append('HTML DIV').attr('id', 'hover');

	// Fills in the static text and placeholder elements for the hover
	function addContent(name, text) {
		dv.html.hover[ARGUMENT] = dv.html.hover.main.append('HTML DIV');
		dv.html.hover[ARGUMENT].append('HTML SPAN').attr('class', 'value');
		if (ARGUMENT) { dv.html.hover[ARGUMENT].append('html:span').attr('class', 'descr').text(ARGUMENT); }
	}
...
dv.update.hoverShow = function(d) {
	...
	dv.html.hover.country.select('CSS SELECTOR').text(COUNTRY NAME);
	dv.html.hover.gdp.select('CSS SELECTOR').text(DOLLAR-FORMATTER(GDP-VALUE));
	dv.html.hover.population.select('CSS SELECTOR').text(BIG-FORMATTER(POPULATION-VALUE / 1000000) + 'M');
	dv.html.hover.life.select('CSS SELECTOR').text(FIX1-FORMATTER(LIFE-VALUE));
	dv.html.hover.fertility.select('CSS SELECTOR').text(FIX1-FORMATTER(FERTILITY-VALUE);
};
